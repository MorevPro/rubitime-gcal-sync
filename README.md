...
![Schedule Synchronization](images/sequence-schedule.png)
...
- **ID формат**: `availableYYYYMMDDHHMM` (например, `available202607081800`)
...
![Webhook Processing](images/sequence-webhook.png)
...
- В описание добавляются: данные записи, информация о клиенте, услуга, цена, комментарии и ссылки на журнал и админку
- В поле **Источник** события добавляется ссылка на внутреннюю админку (Админка) для быстрого перехода из календаря
- **ID записей через webhook** имеет префикс `booked` (например, `booked00123`) для отличия от других типов событий

### Пример созданного события в Google Calendar

```
Summary: ✅ Иван Иванов | 3 500 ₽
Location: Клиника на Пушкина
Source: https://rubitime.ru/profile/analytic/history/8588073 (Админка)
Time: 2026-07-15 14:00-15:00 (Europe/Moscow)

Оплачено: Да
Заказана услуга: Консультация врача по цене 3 500 ₽
Имя клиента: Иван Иванов
Телефон клиента: +79991234567
Email клиента: ivan@example.com
Дата рождения: 1990-05-15
Пол: Мужской
Комментарий клиента: Первичный прием

Открыть запись:  https://rubitime.ru/profile/analytic/history/8588073
```
...
├── images/
│   ├── sequence-webhook.puml     # Диаграмма обработки webhook
│   └── sequence-schedule.puml    # Диаграмма синхронизации слотов
├── Dockerfile
├── docker-compose.yml
├── requirements.txt
...
| `POST` | `/webhook` | Accept Rubitime webhook events |
...
```json
{
  ...
  "data": {
    ...
    "record": "2026-07-15 14:00:00",
    "price": 3500,
    "status": 0,
    "status_title": "Записан",
    "branch_title": "Клиника",
    "service_title": "Услуга",
    "url": "https://rubitime.ru/...",
...
```json
{
  "id": "booked00012",
  "summary": "✅ Иван Иванов | 3 500 ₽",
  "description": "Оплачено: Да\nУслуга: Услуга по цене 3 500 ₽\nИмя клиента: Иван Иванов\nТелефон клиента: +79991234567\nEmail клиента: ivan@example.com\nКомментарий клиента: Первичный прием\n\nОткрыть запись: https://rubitime.ru/profile/analytic/history/12345",
  "location": "Клиника на Пушкина",
  "start": {"dateTime": "2026-07-15T14:00:00+03:00", "timeZone": "Europe/Moscow"},
  "end": {"dateTime": "2026-07-15T15:00:00+03:00", "timeZone": "Europe/Moscow"},
  "source": {"title": "Открыть запись", "url": "https://rubitime.ru/profile/analytic/history/12345"},
  "extendedProperties": {
    "private": {
      "rubitime_record_id": "12345",
      "rubitime_event": "event-update-record"
    }
  }
}
```
...
- Числовые поля (`price`, `status`, `duration`) в веб-хуках принимаются как строки и как числа (Особенность Rubitime API)
